...
const DateRangePicker: React.FC<DateRangePickerProps> = ({ value, onChange }) => {
  ...
  const years = Array.from({ length: 100 }, (_, i) => new Date().getFullYear() - 50 + i); // Last 50 years and next 50 years

  const daysOfWeek = ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su']; // Updated order for Monday-start week
  ...
  const renderDays = (startDate: Date) => {
    ...
    // Calculate the number of empty cells to prepend, shifting the start of the week to Monday
    const offset = (firstDayOfMonth.getDay() + 6) % 7; // Adjust for Monday as the first day
    ...
    for (let date = firstDayOfMonth; date <= lastDayOfMonth; date.setDate(date.getDate() + 1)) {
      ...
      const isSelected = isSameDay(dateCopy, range.startDate) || isSameDay(dateCopy, range.endDate);
      const isInRange =
        range.startDate &&
        range.endDate &&
  ...
  const DroperIcon = ({ direction }) => {
    return (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200" width="10" height="22" className='mb-2 ml-2'>
      {
        direction ?
    ...
    <div className="absolute w-[15vw] top-full left-0 font-mulish bg-white border rounded-lg shadow-lg p-4 mt-2 z-50">
      <div className="flex justify-center mb-4">
        ...
        {/* <select
              ...
              className="ml-2 px-2 py-1 outline-none"
            >
              {years.map((year) => (
